...
//Get cpu structure which the caller runs on
...
growproc(int n)
{
	...
	top = curproc->sz + USERBASE;
	if(n > 0){
		if((top = allocuvm(curproc->pgdir, top, top + n)) == 0)
			return -1;
	} else if(n < 0){
		if((top = deallocuvm(curproc->pgdir, top, top + n)) == 0)
			return -1;
	}
...
fork(void)
{
	...
	*np->tf = *curproc->tf;
	...
	strncpy(np->name, curproc->name, sizeof(curproc->name));
